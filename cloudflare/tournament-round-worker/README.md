...
Даты указываются строго в формате `YYYY-MM-DD` по UTC. Суббота и воскресенье пропускаются автоматически для любых активов. После расчёта Worker закрывает текущий раунд и создаёт следующий на ближайший торговый день. Если `season.endsAt` уже наступил, Worker вместо этого в том же атомарном commit закрывает сезон (`status: "closed"`) и новый раунд не создаёт. Например, после пятничного раунда он создаст открытый раунд на понедельник только если сезон ещё продолжается; до его `startsAt` приложение покажет обратный отсчёт открытия голосования.
