...
import { buildCapitalAssetPerformance, buildCapitalSidePerformance, buildCapitalGrowthBreakdown, type CapitalAssetPerformance } from '../analytics/capitalGrowthRate'
...
<template>
  <section class="mt-16">
    <div v-if="pnlValues.length" class="mt-8">
      <div class="font-serif text-[12px] uppercase tracking-[0.2em] text-white/75">{{ label('I · Profit structure', 'I · Структура прибыли') }}</div>
    ...
    <div v-if="sidePerformance.length" class="mt-12">
      <div class="font-serif text-[12px] uppercase tracking-[0.2em] text-white/75">{{ label('II · Long / Short distribution', 'II · Распределение Long / Short') }}</div>
    ...
    <div v-if="breakdownRows.length" class="mt-12">
      <div class="font-serif text-[12px] uppercase tracking-[0.2em] text-white/75">{{ label('III · Growth rates by scenario and condition usage', 'III · Темпы роста по использованию сценариев и условий') }}</div>
