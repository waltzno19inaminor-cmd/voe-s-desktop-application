...
const getEntryPositionNumber = inject('tradeState').getEntryPositionNumber;
              ...
              <div class="flex-1 overflow-y-auto custom-scrollbar pr-4 min-h-0 pb-10">
                ...
                <div v-if="activeProtocolTab === 'PYRAMIDING' || activeProtocolTab === 'AVERAGING_DOWN'" class="flex flex-col gap-4 transition-all">
                  <div v-for="(ent, idx) in activeMultipleEntries" :key="ent.id" class="flex items-center gap-4">
                     <span class="text-[8px] font-mono opacity-40 font-black tracking-widest w-6">#{{ getEntryPositionNumber(ent) }}</span>
    ...
    <Teleport to="body">
      <Transition name="nier-fade">
        <div v-if="isTemporalOpen" 
             ...
             class="fixed inset-0 z-[11000] flex items-center justify-center p-20 bg-black/20 dark:bg-black/40 backdrop-blur-md cursor-pointer">
